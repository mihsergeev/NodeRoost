...
**Сертификаты для этих имён.** У имени есть галочка с замком: панель заказывает
сертификат Let's Encrypt, ключ при этом генерится на самой ноде и никуда с неё не
уезжает — наверх уходит только запрос на подпись. Нужна одна DNS-запись, один раз
(маска внутренних имён на адрес панели): ни API-ключей DNS-хостинга, ни плагинов к
прокси. Продление идёт само.
