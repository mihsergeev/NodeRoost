...
**Имена внутри сети.** Сервис, закрытый вайтлистом или вовсе без открытых наружу
портов, доступен по своему обычному имени: панель раздаёт нодам запись «имя →
адрес в сети». Публичный DNS при этом не меняется — снаружи имя ведёт туда же,
куда вело. Адрес не запоминается, а берётся у ноды, так что переподключение ноды
записи не ломает.

**Сертификаты для этих имён.** У имени есть галочка «сертификат»: панель
подписывает его своим корнем, а ключ генерится на самой ноде и никуда с неё не
уезжает — наверх уходит только запрос на подпись. Ни домена, ни DNS-записи, ни
открытого порта, ни интернета: имя может быть любым, хоть `nas.mesh`. Корень
панель раскладывает по нодам сама, и подписывать он умеет только выдуманные
внутренние домены: все настоящие ему запрещены, поэтому подделать чужой сайт для
ваших машин панель не может, а новый проект не требует ничего перевыпускать.
Продление идёт само.
