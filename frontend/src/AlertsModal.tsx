...
export function AlertsModal({ onClose, onUnauthorized }: Props) {
  ...
  async function save() {
    ...
    setMsg(null)
    try {
      await putAlerts({ telegram_token: token, telegram_chat: chat, telegram_api: apiUrl, webhook })
      setMsg(t('Сохранено.'))
    } catch (err) {
  ...
  async function runTest() {
    ...
    setMsg(null)
    try {
      // сначала сохраняем — тест идёт по текущим полям
      await putAlerts({ telegram_token: token, telegram_chat: chat, telegram_api: apiUrl, webhook })
        ...
        {loading ? (
          ...
          <>
            <div className="settings-group">
              ...
              <label className="field">
                <span>Chat ID</span>
                <input value={chat} onChange={(e) => setChat(e.target.value)} placeholder="123456789" />
              </label>
              <label className="field">
                <span>{t('Адрес Bot API')}</span>
                <input value={apiUrl} onChange={(e) => setApiUrl(e.target.value)} placeholder="https://api.telegram.org" />
            ...
            <div className="settings-group">
              <h4>{t('Вебхук')}</h4>
              <p className="muted small">
                {t('POST с JSON {"text": "…"} на URL (Slack, Mattermost, свой сервис).')}
              </p>
              <label className="field">
                <span>URL</span>
            ...
            <div className="modal-actions">
              <button className="ghost" onClick={runTest} disabled={testing || !canTest}>
