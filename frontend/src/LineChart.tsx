...
// Свой SVG-график (без внешних либ). Рисуем в реальных пикселях (viewBox =
// измеренная ширина), чтобы линия и подписи не расплющивались.
  ...
  if (points.length < 2) {
    return (
      <div className="chart-empty muted small">
        {t('пока мало данных для графика')}
      </div>
    )
  }
    ...
    <div className="linechart" ref={ref} style={{ height }}>
      {W > 0 && (
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
          ...
          <polygon points={areaPath} fill="url(#lc-grad)" />
          <polyline
            points={line}
            fill="none"
            stroke={color}
            ...
            strokeLinecap="round"
          />
          <circle className="chart-dot" cx={x(last.t)} cy={y(last.v)} r="3.5" fill={color} />
