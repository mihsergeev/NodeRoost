...
class CaInfo(BaseModel):
    ...
    # Домены, разрешённые корню СТАРОГО образца (пусто у нынешнего — он вместо
    # этого запрещает публичные).
    suffixes: list[str] = []
    # Сколько публичных доменов верхнего уровня корню запрещено
    blocked: int = 0
    # Команды установки сертификата на машину (пусто, если адрес control-сервера
    # не задан — без него ссылке неоткуда взяться)
    install_sh: str = ""
    install_ps1: str = ""
...
class CaSettingsIn(RequestModel):
    """Автоустановка корня и — по явной просьбе — его перевыпуск."""

    auto: bool = True
    # ПЕРЕВЫПУСТИТЬ корень. Отдельным флагом, а не побочным эффектом сохранения
    # галочки: перевыпуск обесценивает всё, что подписано, и заставляет обойти
    # ноутбуки и телефоны заново.
    reissue: bool = False
