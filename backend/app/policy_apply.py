...
# Замок на пару «сохранить правила → запушить политику». Два запроса, пришедшие
# одновременно, раскладывались как «A сохранил, B сохранил, B запушил, A
# запушил»: в панели оставались правила B, а в сети действовали правила A.
# Расходилось это до следующего цикла самоисцеления — минуту панель показывала
# не тот доступ, который на самом деле работает. Бэкенд — один процесс, поэтому
# asyncio-замка достаточно.
_write_lock = asyncio.Lock()


def write_lock() -> asyncio.Lock:
    """Взять перед изменением состояния, влияющего на политику, и держать до пуша."""
    return _write_lock
